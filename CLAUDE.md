...
Multi-token Crypto Price Tracker — fetches OHLCV candle data for **BTC, ETH, SOL, XRP** (USDT pairs) from KuCoin via CCXT, computes 52 technical indicators, and stores results in MongoDB Atlas. Runs autonomously via GitHub Actions (hourly and daily cron jobs) or optionally via GCP Cloud Run.
...
6. **Bulk upsert** only newly fetched rows into MongoDB, skipping any with NaN indicators

### MongoDB Schema

- Database: `btc_data` (production), `btc_data_test` (testing)
- Collection naming: `{token}_1h_price_data`, `{token}_daily_price_data`
  - e.g. `btc_1h_price_data`, `eth_daily_price_data`, `sol_1h_price_data`, `xrp_daily_price_data`
- Each document is keyed by `timestamp` (UTC datetime); unique index enforced
- Legacy collections (`1h_price_data`, `daily_price_data`) still exist during migration

### Key Modules (`btc_tracker_mongodb/`)

| File | Purpose |
|---|---|
| `config.py` | Central config: TOKENS, TIMEFRAMES, DB names, collection name mapping |
...
| `indicators.py` | Single source of truth for all 52 indicators: compute_all(), get_numeric_cols() |
...
### Legacy modules (preserved during migration, will be removed in cleanup)

| File | Purpose |
|---|---|
| `seed_historical.py` | Old one-time backfill: 500 hourly BTC candles |
| `seed_daily.py` | Old one-time backfill from `daily_history.csv` |
| `update_hourly.py` | Old incremental hourly BTC updater |
| `update_daily.py` | Old incremental daily BTC updater |
| `mongodb_query.py` | Old debug query (latest 10 hourly) |
| `query_latest_daily.py` | Old debug query (latest 100 hourly — misnamed) |

### Technical Indicators (52 columns)

Computed by `indicators.py` using `pandas-ta-classic`:

**Trend:** SMA (50, 100, 200), EMA (20, 50, 100, 200), Ichimoku (9, 26, 52), ADX / +DI / -DI (14)
**Momentum:** RSI (14), Stochastic RSI (14, K=3, D=3), MACD (12, 26, 9), Williams %R (14), CCI (20), ROC (12, 24)
**Volatility:** Bollinger Bands (20, 2sigma), Donchian Channel (20), ATR (14), Parkinson Vol (14), Realized Vol (14, 30), Vol Ratio (14/30)
**Price levels:** Fibonacci retracement (rolling 50-period), VWAP (cumulative)
**Custom:** HDPR (mean-reversion, MA50, 3% threshold)
...
- The `de421.bsp` ephemeris file (16 MB) is legacy — used by old scripts for moon phase. Will be deleted in cleanup.
