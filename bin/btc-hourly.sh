#!/usr/bin/env bash
# btc-hourly.sh — Hourly BTC data pipeline
...
until docker exec crypto_research_assistant-mongo-1 mongosh --eval "db.adminCommand('ping')" --quiet > /dev/null 2>&1; do
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        notify_failure "Hourly Update (BTC)" "🗄 MongoDB not reachable after ${MAX_WAIT}s
🔧 Is Docker running?"
        exit 1
    fi
done
...
echo "=== BTC 1h — $(date -u +%H:%M:%S) ===" >> "${LOG_FILE}"

if ! "${VENV}" "${PROJECT_DIR}/update.py" --symbol BTC-USDT --timeframe 1h >> "${LOG_FILE}" 2>&1; then
    FAILED="${FAILED}spot-1h "
fi

if ! "${VENV}" "${PROJECT_DIR}/update.py" --symbol BTC-USDT --timeframe 1h --market-type perp >> "${LOG_FILE}" 2>&1; then
...
if [ -n "${FAILED}" ]; then
    ...
    notify_failure "Hourly Update (BTC)" "🪙 Token: BTC-USDT
❌ Failed: ${FAILED}
